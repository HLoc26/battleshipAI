...
def mainMenuScreen(window):
    ...
    for button in BUTTONS:
        if button.name in ['Probability', 'Genetic Alg.']:
            button.active = True
            button.draw(window)
        else:
            button.active = False
...
def endScreen(window):
    ...
    for button in BUTTONS:
        if button.name in ['Probability', 'Genetic Alg.', 'Quit']:
            button.active = True
            button.draw(window)
        else:
            button.active = False
...
BUTTONS = [
    ...
    Button(BUTTONIMAGE1, (250, 100), (SCREENWIDTH // 2 - 300, SCREENHEIGHT // 2), 'Probability'),
    Button(BUTTONIMAGE1, (250, 100), (SCREENWIDTH // 2 + 50, SCREENHEIGHT // 2), 'Genetic Alg.')
]
            ...
            if event.button == 1:
                ...
                for button in BUTTONS:
                    if button.rect.collidepoint(pygame.mouse.get_pos()):
                        ...
                        elif (button.name == 'Probability' or button.name == 'Genetic Alg.') and button.active == True:
                            if button.name == 'Probability':
                                computer = EasyComputer()

                            elif button.name == 'Genetic Alg.':
