...
class BattleshipGame:
    ...
    def check_hit(self, x: int, y: int) -> Tuple[bool, Optional[Ship]]:
        """Kiểm tra hit và trả về tàu bị trúng (nếu có)"""
        is_hit = self.board[x][y] != ' '
        hit_ship = None
        if is_hit:
...
class BattleshipAI:
    def __init__(self, board_size: int = 10):
        ...
        self.current_hunt_ship = None  # Theo dõi tàu đang săn

    
    def register_sunk_ship(self, ship: Ship):
        """Đăng ký tàu đã bị đánh chìm"""
        # Xóa tàu khỏi danh sách còn lại
        if ship.name in self.remaining_ships:
            del self.remaining_ships[ship.name]
        
        # Đánh dấu tất cả các ô của tàu là đã xác nhận
        for pos in ship.positions:
            self.confirmed_ship_cells.add(pos)
        
        # Reset trạng thái săn nếu đang săn tàu này
        if self.current_hunt_ship and ship.length == self.current_hunt_ship:
            self.current_hunt_ship = None
            self.hunting_direction = None
            self.hunt_stack.clear()
        
        # Cập nhật lại bản đồ xác suất
    ...
    def play_game(self) -> List[Tuple[int, int, bool]]:
        ...
        while len(moves) < self.board_size * self.board_size:
            ...
            if len(self.hits) >= sum(self.ships.values()):
                break
        return moves
    def play_complete_game(self) -> List[Tuple[int, int, bool, Optional[Ship]]]:
        ...
        with open("moves.txt", "w", encoding="utf-8") as f:
            ...
            f.write("\nMoves:\n")
            f.write("Move  Position  Result  Sunk Ship  Board State\n")
            f.write("-" * 60 + "\n")
            ...
            while move_count < self.board_size * self.board_size:
                ...
                # Thêm thông tin về tàu bị đánh chìm
                if sunk_ship:
                    f.write(f"\nSunk {sunk_ship.name}!\n")
                    f.write(f"Remaining ships: {list(self.remaining_ships.keys())}\n")
                    f.write("-" * 60 + "\n")
