...
class BattleshipAI:
    ...
    def update_late_game_probabilities(self):
        """Update probability map with enhanced late game strategy"""
        # Reset probability map
        self.probability_map = np.zeros((self.board_size, self.board_size))
        
        # First pass: Calculate base probabilities using ship density
        for i in range(self.board_size):
            for j in range(self.board_size):
                if (i, j) not in self.hits and (i, j) not in self.misses:
                    self.probability_map[i, j] = self.calculate_ship_density(i, j)

        # Second pass: Significantly boost probabilities near unconfirmed hits
        if self.unconfirmed_hits:
            for hit_x, hit_y in self.unconfirmed_hits:
                # Get valid directions for ship placement based on adjacent hits
                valid_directions = self._get_valid_directions(hit_x, hit_y)
                
                # Boost probabilities in valid directions
                for direction in valid_directions:
                    dx, dy = direction.value
                    next_x, next_y = hit_x + dx, hit_y + dy
                    
                    # Check multiple spaces in the direction (up to max ship length)
                    for distance in range(1, self.max_remaining_ship_size):
                        check_x = hit_x + (dx * distance)
                        check_y = hit_y + (dy * distance)
                        
                        if not (0 <= check_x < self.board_size and 0 <= check_y < self.board_size):
                            break
                        
                        if (check_x, check_y) in self.misses or (check_x, check_y) in self.sunk_ship_positions:
                            break
                        
                        if (check_x, check_y) not in self.hits:
                            # Higher boost for closer squares
                            boost_factor = 4.0 / distance  # Decreasing boost with distance
                            self.probability_map[check_x, check_y] *= boost_factor

        # Third pass: Consider minimum remaining ship size and invalid positions
        invalid_positions = set()
        for i in range(self.board_size):
            for j in range(self.board_size):
                if (i, j) not in self.hits and (i, j) not in self.misses:
                    can_fit_ship = False
                    # Check each remaining ship length
                    for ship_length in self.remaining_ships.values():
        ...
        # Fourth pass: Pattern recognition for consecutive hits
        self._apply_pattern_recognition()
        
        # Normalize probabilities if any exist
        if np.sum(self.probability_map) > 0:
            self.probability_map /= np.sum(self.probability_map)

    def _get_valid_directions(self, x: int, y: int) -> List[Direction]:
        """Determine valid directions for ship placement based on existing hits"""
        valid_directions = []
        consecutive_hits = self._get_consecutive_hits(x, y)
        
        if len(consecutive_hits) > 1:
            # If we have multiple hits in a line, only consider that line's direction
            dx = consecutive_hits[1][0] - consecutive_hits[0][0]
            dy = consecutive_hits[1][1] - consecutive_hits[0][1]
            if dx != 0:
                valid_directions = [Direction.NORTH, Direction.SOUTH]
            elif dy != 0:
                valid_directions = [Direction.EAST, Direction.WEST]
        else:
            # Otherwise, consider all directions
            valid_directions = list(Direction)
        
        # Filter out directions that would hit invalid spaces
        return [d for d in valid_directions if self._is_valid_direction(x, y, d)]

    def _get_consecutive_hits(self, x: int, y: int) -> List[Tuple[int, int]]:
        """Find all consecutive hits connected to the given position"""
        hits = [(x, y)]
        checked = {(x, y)}
        to_check = [(x, y)]
        ...
        while to_check:
            ...
            for direction in Direction:
                ...
                if ((next_x, next_y) in self.unconfirmed_hits and 
                    (next_x, next_y) not in checked):
                    hits.append((next_x, next_y))
                    checked.add((next_x, next_y))
                    to_check.append((next_x, next_y))
        
        return hits

    def _is_valid_direction(self, x: int, y: int, direction: Direction) -> bool:
        """Check if a direction is valid for ship placement"""
        dx, dy = direction.value
        next_x, next_y = x + dx, y + dy
        
        return (0 <= next_x < self.board_size and 
                0 <= next_y < self.board_size and
                (next_x, next_y) not in self.misses and
                (next_x, next_y) not in self.sunk_ship_positions)

    def _apply_pattern_recognition(self):
        """Apply pattern recognition to enhance probability calculation"""
        # Find all groups of consecutive hits
        hit_groups = []
        checked_hits = set()
        
        for hit in self.unconfirmed_hits:
            if hit not in checked_hits:
                group = self._get_consecutive_hits(*hit)
                hit_groups.append(group)
                checked_hits.update(group)
        
        # Analyze each group and boost probabilities accordingly
        for group in hit_groups:
            if len(group) > 1:
                # Determine if group is horizontal or vertical
                is_horizontal = group[0][0] == group[1][0]
                
                # Calculate potential ship positions
                for ship_length in self.remaining_ships.values():
                    if len(group) <= ship_length:  # Ship could contain this group
                        # Check possible placements that would cover the group
                        for start_pos in self._get_possible_starts(group, ship_length, is_horizontal):
                            # Boost probabilities for all positions in this possible placement
                            self._boost_ship_placement(start_pos, ship_length, is_horizontal)

    def _get_possible_starts(self, group: List[Tuple[int, int]], ship_length: int, 
                            is_horizontal: bool) -> List[Tuple[int, int]]:
        """Get all possible starting positions for a ship that would cover the hit group"""
        min_pos = min(pos[1] if is_horizontal else pos[0] for pos in group)
        max_pos = max(pos[1] if is_horizontal else pos[0] for pos in group)
        fixed_pos = group[0][0] if is_horizontal else group[0][1]
        
        starts = []
        for start in range(max_pos - ship_length + 1, min_pos + 1):
            if is_horizontal:
                if self._is_valid_ship_placement(fixed_pos, start, ship_length, True):
                    starts.append((fixed_pos, start))
            else:
                if self._is_valid_ship_placement(start, fixed_pos, ship_length, False):
                    starts.append((start, fixed_pos))
        
        return starts

    def _is_valid_ship_placement(self, x: int, y: int, length: int, 
                            is_horizontal: bool) -> bool:
        """Check if a ship placement is valid"""
        for i in range(length):
            check_x = x if is_horizontal else x + i
            check_y = y + i if is_horizontal else y
            
            if not (0 <= check_x < self.board_size and 0 <= check_y < self.board_size):
                return False
            
            if ((check_x, check_y) in self.misses or 
                (check_x, check_y) in self.sunk_ship_positions):
                return False
        
        return True

    def _boost_ship_placement(self, start_pos: Tuple[int, int], length: int, 
                            is_horizontal: bool):
        """Boost probabilities for a possible ship placement"""
        boost_factor = 2.0  # Base boost factor
        x, y = start_pos
        ...
        for i in range(length):
            ...
            if ((check_x, check_y) not in self.hits and 
                (check_x, check_y) not in self.misses):
                self.probability_map[check_x, check_y] *= boost_factor
